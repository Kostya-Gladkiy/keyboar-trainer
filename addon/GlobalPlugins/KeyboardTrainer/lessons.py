...
class Category:
    ...
    def check_record(self):
        ...
        suma = len(self.successfuls)+len(self.mistakes)
        try:
            errors = len(self.mistakes)/(suma/100)
        except Exception:
            errors = 0
        ...
        if ch == " " and self.entered_text == self.word:
            ...
            if self.entered_text != "":
                self.mistakes.append(
                    "#"*self.get_count_errors(self.word, self.entered_text))
            self.entered_text = ""
...
class NumericalDictation(ByWordsDictation):
    ...
    def __init__(self):
        ...
        d = list(range(1000, 10000))
        for item in [a, b, c, d]:
            random.shuffle(item)
        self.words = a + b[:20] + c[:30] + d[:40]
...
class TextDictation(Category):
    key_in_config_max_count = "maximum number of characters in text dictation in Ukrainian"
    sentence = []
    entered_text = ""
    message_on_exit = "Текстовий диктант завершено"

    def __init__(self):
        f = open(BASE_DIR+"/text/uk/text.txt", "r", encoding="utf-8")
        self.text = f.read()
        f.close()
